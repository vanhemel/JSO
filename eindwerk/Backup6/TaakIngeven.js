	...
	$.ajax({
		...
		success: function (data) {
			...
			for (var i = 0; i < data.result.length; i++) {
				if (data.result[i].ID == 2) { 			// ID van IRINA ipv 1
...
function lijstVullen() {
	...
	$.ajax('http://127.0.0.1:2001/catWeergeven', {
		...
		success: function (data) {
			$("#takenLijst").empty();
			var hoofdcats = data.result.filter(cat => cat.PARENTCATIDC == undefined && cat.ID == 2);	// ID van IRINA ipv 1
			var unic_hoofdcats = [];
...
function verwerksubcat(parentid, data) {
	var subcats = data.filter(cat => cat.PARENTCATIDC == parentid && cat.ID == 2);		// ID van IRINA ipv 1
	var unic_subcats = [];
...
function verwerkTaken(result) {
	var taken = result.filter(t => t.PARENTCATIDT != undefined && t.ID == 2);			// ID van IRINA ipv 1
